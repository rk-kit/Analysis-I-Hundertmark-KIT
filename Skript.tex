...
\begin{document}
    ...
    \subsection{[*] Der Raum $\realnumbers^d$ und Normen}
    ...
    \horizontalline
    Im $\realnumbers$ haben wir Konvergenz über den Betrag definiert. Im $\realnumbers^d$ benötigen wir daher ein ähnliches Konzept. Wir betrachten dafür zunächst einige Beispiele solcher \textit{Normen}.
    ...
    \begin{definition}[Norm] % Definition 1
        ...
        \begin{align*}
            \norm{.}: \realnumbers^d\fromto \realnumbers
        \end{align*}
        mit folgenden Eigenschaften:
        \begin{enumerate}[label=\alph*)]
            \item $\norm{x} \geq 0~\forall x\in\realnumbers^d$ sowie $\norm{x} = 0\impl x=0$
            \item $\forall\lambda\in\realnumbers,~x\in\realnumbers^d\colon \norm{\lambda x} = \abs{\lambda}\cdot\norm{x}$
            \item $\forall x,y\in\realnumbers^d\colon\norm{x+y} \leq \norm{x} + \norm{y}$\quad\text{(Dreiecksungleichung)}
        \end{enumerate}
    \end{definition}
    ...
    \begin{definition}[Äquivalenz von Normen]
        ...
        \begin{align*}
            \exists c_1, c_2\in\realnumbers\colon c_1\cdot\norm{x}_a \leq \norm{x}_b \leq c_2 \norm{x}_a\quad\forall x\in V
        \end{align*}
    \end{definition}
    ...
    \subsection{[*] Konvergenz im $\realnumbers^d$}
    ...
    \begin{definition}[Konvergenz im $\realnumbers^d$] % Definition 2
        Sei $(x_n)_n$ eine Folge in $\realnumbers^d$ mit $x_n\in\realnumbers^d~\forall n$. Dann konvergiert $(x_n)_n$ gegen $x\in\realnumbers^d$, falls
        \begin{alignat*}{2}
    ...
    \begin{beobachtung}[Konvergenz in $\realnumbers$ vs $\realnumbers^d$]
        Es sei ${x_n}\in\realnumbers^d$ mit $x_n = \pair{x_n^1, x_n^2, \dots, x_n^d}$\footnote{Koordinaten von $(x_n)_n$}. Damit ergeben sich die Folgen $(x_n^1)_n$, $(x_n^2)_n$, \dots, $(x_n^d)_n$ in $\realnumbers$.
    ...
    \begin{definition}[Exponentialfunktion]
        $z\in\complexnumbers$, $e^z = \exp(z)\definedas \sum_{n=0}^{\infty} \frac{z^n}{n!}$, $z^0=1$. $\exp: \complexnumbers\fromto\complexnumbers$.
    \end{definition}

    \begin{satz}
        \theoremescape
        \begin{enumerate}[label=(\alph*)]
            \item Für alle $z\in\complexnumbers$ konvergiert obige Reihe absolut
            \item $\exp(z_1)\cdot\exp(z_2) = \exp(z_1+z_2)$
            \item $\exp(z)\neq 0~\forall z\in\complexnumbers$ und $\exp(z)^{-1} = \exp(-z)$
            \item $\overline{\exp(z)} = \exp(\overline{z})$. Insbesondere ist $\abs{\exp(z)} = \exp(\re(z))$
            \item Für $\alpha\in\realnumbers$ ist $\abs{\exp(i\alpha)}=1$. Setzen
            \begin{align*}
                \cos \alpha &= \cos(\alpha) \definedas \re (e^{i\alpha})\\
                &= \frac{1}{2} \cdot\pair{e^{i\alpha} + e^{-i\alpha}}
                \sin \alpha &= \sin(\alpha) \definedas \im (e^{i\alpha}) = \frac{1}{2i}\cdot\pair{e^{i\alpha}-e^{-i\alpha}}
                \intertext{Haben}
                -1 &\leq \cos \alpha \leq 1\\
                -1 &\leq \sin \alpha \leq 1\\
                \cos(\alpha)^2 + \sin(\alpha)^2 &= 1\quad\forall \alpha\in\realnumbers\\
                \text{ Euklidische Gleichung } e^{i\alpha} &= \cos(\alpha) + i\sin(\alpha)
            \end{align*}
        \end{enumerate}
        ...
        \begin{proof}[(a)]
            \begin{align*}
                ...
                \frac{a_{n+1}}{a_n} &= \frac{z}{n+1}\fromto 0 \text{ für } n\fromto\infty\\
                \annot{\impl}{Quotientenkriteirum} \sum_{n=0}^{\infty} a_n &= \sum_{n=0}^{\infty} \frac{z^n}{n!}\text{ konvergiert absolut }
            \end{align*}
        \end{proof}
        \begin{proof}[(b)]
            \begin{align*}
                e^z + e^{zi} &= \pair{\sum_{j=0}^{\infty} \frac{z^j}{j!}} \cdot \pair{\sum_{k=0}^{\infty} \frac{z^k}{k!}}\\
                &= \text{ Cauchy-produkt } \\
                &= \sum_{n=0}^{\infty} \sum_{\nu=0}^{n} a_{\nu} b_{n-\nu}\\
                &= \sum_{n=0}^{\infty} \sum_{\nu=0}^{n} \frac{(z_n)^{\nu}}{\nu!} \frac{(z_n)^{n-\nu}}{(n-\nu)!}\\
                &= \sum_{n=0}^{\infty} \frac{1}{n!} \sum_{\nu=0}^{n} \frac{n!}{\nu!(n-\nu)!} (z_1)^{\nu} z_z)^{n-\nu}\\
                &= \sum_{\nu=0}^{n} \binom{n}{\nu} (z_1)^{\nu} (z_2) ^{n-\nu} = (z_1+z_2)^n\\[10pt]
        ...
        \begin{proof}[(c)]
            \begin{align*}
                \sum_{k=0}^{n} \frac{z^k}{k!} &= \sum_{k=0}^{n} \frac{\overline{z^k}}{k!}\\
                &= \sum_{k=0}^{n} \frac{\overline{z}^k}{k!} \fromto \exp(\overline{z})\\[10pt]
                \overline{\exp(z)} &\impl \overline{\exp(z)} = \exp{\overline{z}}
                \intertext{Insbesondere}
                e^z e^{-z} &= e^{z-z} = e^0 = 1\\
                \impl e^z e^{-z} \neq 0 \quad \forall z\in\complexnumbers
                \intertext{und}
                (e^z)^{-1} &= e^{-z}
            \end{align*}
        \end{proof}
        \begin{proof}
            \begin{align*}
                \abs{\exp(z)}^2 &= \overline{\exp(z)} \exp(z)\\
                &= \exp(\overline{z}) \exp(z)\\
                &= \exp(\overline{z} + z) = \exp(z \re(z))\\
                &= \pair{\exp(\re(z))}^2\\[10pt]
                \impl \abs{\exp(z)} &= \abs{\exp(\re(z))} = \exp(\re(z))
                \intertext{Behauptung: $e^x = \exp(z) > 0~\forall x\in\realnumbers$}
                \text{ Ist } x \geq 0 &\impl \exp(z) = \sum_{n=0}^{\infty} \frac{x^n}{n!}= 1 + \sum_{n=1}^{\infty} \frac{x^n}{n!}\geq 1\\
                \text{ Ist } x < 0 &\impl -x > 0\quad \exp(z) = \frac{1}{\exp(-x)} > 0
            \end{align*}
        \end{proof}

        \begin{proof}[(d)]
            Es sei $\alpha\in\realnumbers$.
            \begin{align*}
                \abs{e^{i\alpha}}^2 &= \overline{e^{i\alpha}} e^{i\alpha}\\
                &= e^{-i\alpha} e^{i\alpha} = e^0 = 1\\[10pt]
                \re(e^{i\alpha}) &= \frac{1}{2}\cdot\pair{e^{i\alpha} + \overline{e^{-i\alpha}}}\\
                &= \frac{1}{2}\cdot\pair{e^{i\alpha} + e^{-i\alpha}}\\
                &\definedasbackwards \cos \alpha\\[10pt]
                \im(e^{i\alpha}) &= \frac{1}{2i}\cdot\pair{e^{i\alpha} - \overline{e^{-i\alpha}}}\\
                &= \frac{1}{2i}\cdot\pair{e^{i\alpha} - e^{-i\alpha}}\\[10pt]
                \abs{e^{i\alpha}}^2 &= \pair{\re(e^{i\alpha})}^2 + \im(e^{i\alpha})^2\\
                &= \pair{\cos \alpha}^2 + \pair{\sin \alpha}^2\\[10pt]
                e^{i\alpha} &= \re (e^{i\alpha}) + i \im (e^{i\alpha)} = \cos \alpha + i \cdot \sin \alpha
            \end{align*}
        \end{proof}
    \end{satz}
    ...
    \subsection{}
    \thispagestyle{pagenumberonly}
    Wir untersuchen Reihen der Form $ \sum_{n=0}^{\infty} a_n\cdot z^n$ oder $ \sum_{n=0}^{\infty} a_n \cdot \pair{z-z_0}^n$, $z_0\in\complexnumbers$ fest, $z\in\complexnumbers$ oder $\realnumbers$, $a_n\in\complexnumbers$.\\
    Partialsummen:
    \begin{align*}
    ...
    \begin{definition}[Konvergenzradius]
        \begin{align*}
            R &\definedas \sup \set{\abs{z}: z\in\complexnumbers \text{ und } \sum_{n=0}^{\infty} a_n \cdot z^n \text{ konvergent } }
        \end{align*}
    ...
    \begin{satz}
        ...
        \begin{align*}
            B_R (a) &= \set{z\in\complexnumbers: \abs{z} < R}
        \end{align*}
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                \impl K\definedas \sup_{n\in\naturalnumbers} \set{a_n z^n} < \infty
                \intertext{Sei $0 < r < \abs{z_1}$, $0 < \theta \definedas \frac{r}{\abs{z_1}} < 1$}
                z &\leq \overline{B_r (a)} = \set{\abs{z} \leq r}\\[10pt]
    ...
    \begin{bemerkung}
        ...
        \begin{align*}
            \sum_{n=0}^{\infty} a_n z^n, \sum_{n=0}^{\infty} a_n \cdot \pair{z-z_0}^n \text{ mit } a_n\in\realnumbers^d \text{ oder } \complexnumbers^d
        \end{align*}
        betrachten. Wir setzen
        \begin{align*}
            R &= \sup \set{\abs{z}: z\in\complexnumbers und \sum_{n=0}^{\infty} a_n z^n \text{ konvergent } }
        \end{align*}
    \end{bemerkung}
    ...
    \begin{lemma}
        ...
        \begin{proof}
            \begin{align*}
                ...
                \impl K &= \sup_{n\in\naturalnumbers_0} \abs{a_n z_1^n} = \sup_{n\in\naturalnumbers} \abs{a_n} \abs{z_1}^n < \infty\\
    ...
    \begin{lemma} % Lemma 4
        ...
        \begin{align*}
            \impl \text{ Für alle } 0 &< r < \abs{z_1}\quad\forall k\in\naturalnumbers_0\\
    ...
    \begin{align*}
        ...
        \intertext{sodass}
        \forall k\in\naturalnumbers_0\colon \abs{\sum_{n=0}^{\infty} a_n z^n} &\leq M_{r,z_1} \abs{\frac{z}{z_1}}^{k+1}\quad\forall \abs{z} \leq r\\
    ...
    \begin{satz} % Satz 5
        ...
        \begin{align*}
            \sum_{n=0}^{\infty} a_n (z_j)^n &= 0\quad\forall j\in\naturalnumbers\\
            \impl a_n &= 0\quad\forall n\in\naturalnumbers_0
        \end{align*}
        \begin{proof}
            Angenommen nicht alle $a_n=0$
            \begin{align*}
                \impl B &\definedas \set{n\in\naturalnumbers_0: a_n \neq 0} \neq \emptyset\\
    ...
    \begin{satz} % Satz 6
        ...
        \begin{proof}
            \begin{align*}
                ...
                \annot{\impl}{Satz 5} c_n &= 0\quad \forall n\in\naturalnumbers_0 \equivalent a_n = b_n \quad\forall n\in\naturalnumbers_0
            \end{align*}
        \end{proof}
    \end{satz}
    ...
    \begin{bemerkung}
        ...
        \begin{align*}
            R &= \sup \set{\abs{z} : z\in\complexnumbers \text{ und } \sum_{n=0}^{\infty} a_n z^n \text{ konvergent } }
        \end{align*}
    \end{bemerkung}
    ...
    \subsection{}
    \thispagestyle{pagenumberonly}
    Sei $D\subseteq \realnumbers$ (oder $D\subseteq \complexnumbers$, wenn komplexe Funktion von $z$) und $D\neq\emptyset$.\\
    $\mathbb{K} = \realnumbers$ oder $\complexnumbers$ (Körper)

    \begin{definition}[$\varepsilon-\delta$ Definition von Stetigkeit]
        Sei $f: D \fromto \mathbb{K}$ und $x_c\in D$.\\
    ...
    \begin{bemerkung}
        ...
        \begin{enumerate}[label=\arabic*)]
            \item $x$ Input, $f(x)$ Output. $f$ stetig in $x_0 \equivalent$ Output $f(x)$ ist $\varepsilon$-nahe bei $f(x_0)$, sofern Input $x$ $\delta$-nahe bei $x_0$ ist.
            \item Eine Funktion $f: D\fromto\realnumbers^d$ oder $\complexnumbers^d$ ist stetig in $x_0\in D$, falls
            \begin{align*}
                \forall \varepsilon>0~\exists\delta>0\colon \norm{f(x)-f(x_0)} &< \varepsilon\\
                \forall x\in D\colon \abs{x-x_0} &<\delta
            \end{align*}
            oder $f:D\fromto V$ mit $V$ Vektorraum mit Norm $\norm{\cdot}$. $f$ ist steig in $x_0$, falls
            \begin{align*}
                \forall \varepsilon>0~\exists\delta>0\colon \norm{f(x)-f(x_0)} &< \varepsilon\\
                \forall x\in D\colon \abs{x-x_0} &<\delta
            \end{align*}
            \item Stetigkeit ist eine lokale Eigenschaft.
            \item Direcklet Funktion
            \begin{align*}
                f(x) &\definedas \begin{cases}
                                     1\quad x\in\mathbb{Q}\\
                                     0\quad x\in\realnumbers\exclude\mathbb{Q}
                \end{cases}
            \end{align*}
            nirgends stetig auf $\realnumbers$.
            \item Für
            \begin{align*}
                f(x) &\definedas \begin{cases}
    ...
    \begin{lemma}
        Ist $f:D\fromto\mathbb{K}$ (oder $\mathbb{K}^d$) stetig in $x_0\in D$.\\
        Dann gilt
        \begin{enumerate}[label=\arabic*.]
            \item $\exists \delta > 0$ mit $\abs{f(x)}\leq 1+\abs{f(x_0)}$~$\forall x\in D$, $\abs{x-x_0} < \delta$
            \item Ist $f(x_0) \neq 0$ dann existiert $\delta>0$.
            \begin{align*}
                \frac{1}{2}\abs{f(x)} &\leq \abs{f(x)} \leq \frac{3}{2} \abs{f(x_0)} \text{ für } x\in D, \abs{x-x_0} < \delta
            \end{align*}
        \end{enumerate}
        ...
        \begin{proof}
            Nehme $\varepsilon = 1$
            \begin{align*}
                \annot{\impl}{Stet.} \exists\delta > 0\colon \abs{f(x)-f(x_0)} &< \varepsilon\quad x\in D, \abs{x-x_0} < \delta\\
                \impl \abs{f(x)} &= \abs{f(x)-f(x_0)+f(x_0)} \leq \underbrace{\abs{f(x)-f(x_0)}}_{< 1} + \abs{f(x_0)}\\
                &\leq 1 + \abs{f(x_0)} \impl 1.
            \end{align*}
        \end{proof}
        \begin{proof}
            Ist $f(x) \neq 0$ wähle $\varepsilon = \frac{1}{2}\abs{f(x_0)} > 0$, $\delta > 0$.
            \begin{align*}
                \abs{f(x)-f(x_0)} &< \varepsilon = \frac{1}{2} \abs{f(x_0)} \quad x\in D, \abs{x-x_0} < \delta\\
                \impl \abs{f(x)} &= \abs{f(x_0)-f(x_0)+f(x)} \leq \abs{f(x_0)} + \abs{f(x_0) + f(x)} \leq \frac{1}{2}\abs{f(x_0)}\\
                \abs{f(x)} &= \abs{f(x_0)+f(x)-f(x_0)} \geq \abs{f(x_0)} = \abs{f(x)-f(x_0)}\\
                &\geq \abs{f(x_0)} = \frac{1}{2} \abs{f(x_0)} = \frac{1}{2}\abs{f(x)}
            \end{align*}
        \end{proof}
    \end{lemma}
    ...
    \begin{notation}
        Seien
        \begin{align*}
            f&: D_f \fromto \mathbb{K}\\
            g&: D_g \fromto \mathbb{K}
        \end{align*}
        Setzen
        \begin{align*}
            f + g: D_{f,g} = D_f \cap D_g &\fromto \mathbb{K}\\
            x &\mapsto  f(x) + g(x)\\[10pt]
            f \cdot g: D_{f,g} &\fromto \mathbb{K}\\
            x &\mapsto  f(x) \cdot g(x)\\[10pt]
            \frac{f}{g}: D_{f,g} \exclude\set{x\in D_g: g(x)=0} &\fromto \mathbb{K}\\
            x &\mapsto  \frac{f(x)}{g(x)}
        \end{align*}
    \end{notation}

    \begin{bemerkung}
        $f+g$ geht auch für $f: D_f \fromto \mathbb{K}^d$, $g: D_f \fromto \mathbb{K}^d$.\\
        $f\cdot g$ und $\frac{f}{g}$ geht auch für $f: D_f \fromto \mathbb{K}^d$, $g: D_f \fromto \mathbb{K}$.
    \end{bemerkung}

    \begin{satz} % Satz 3
        Seien $f, g: D\fromto \mathbb{K}$ stetig in $x_0\in D$.\\
        Dann gilt
        \begin{enumerate}[label=\arabic*.]
            \item $f+g$ ist stetig in $x_0$
            \item $f\cdot g$ ist stetig in $x_0$
            \item Ist $g(x_0)\neq 0$, dann ist $\frac{f}{g}$ stetig in $x_0$
        \end{enumerate}

        \begin{proof}[1.]
            \begin{align*}
                \abs{f(x)+g(x)-(f(x_0)+g(x_0))} &\leq \abs{f(x)-f(x_0)} + \abs{g(x)-g(x_0)}
                \intertext{Geg $\varepsilon >0$}
                \exists \delta_1, \delta_2 > 0\colon \abs{f(x)-f(x_0)} &< \frac{\varepsilon}{2}\tag{$x\in D, \abs{x-x_0} < \delta$}\\
                \abs{g(x)-g(x_0)} &< \frac{\varepsilon}{2}
                \intertext{Ist $\min \set{\delta_1, \delta_2} > 0$}
                \impl \abs{f(x)+g(x)-(f(x_0)+g(x_0))} &< \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon\tag{$x\in D, \abs{x-x_0} < \delta$}
            \end{align*}
        \end{proof}

        \begin{proof}[2.]
            \begin{align*}
                \abs{f(x)\cdot g(x)-(f(x_0)\cdot g(x_0))} &\leq \abs{f(x)-f(x_0)}\cdot\abs{g(x)} + \abs{f(x_0)}\cdot\abs{g(x)-g(x_0)}\\
                \text{ Lemma 2 } \impl \exists \delta_0 &> 0\colon \abs{g(x)} \leq 1 + \abs{g(x_0)} \quad\forall x\in D, \abs{x-x_0} < \delta_0
                \intertext{f,g stetig}
                \forall\varepsilon >0~\exists \delta_1, \delta_2 > 0\colon \abs{f(x)-f(x_0)} &< \frac{\varepsilon}{2(1+\abs{g(x)})}\\
                \abs{g(x)-g(x_0)} &< \frac{\varepsilon}{2(1+\abs{f(x)})}\\
                \delta &\definedas \min(\delta_0, \delta_1, \delta_2)\\
                \impl \abs{f(x)\cdot g(x)-(f(x_0)\cdot g(x_0))} &< \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon\tag{$x\in D, \abs{x-x_0} < \delta$}
            \end{align*}
        \end{proof}
        ...
        \begin{proof}[3.]
            Sei $g(x_0) \neq 0$
            \begin{align*}
                \frac{f(x)}{g(x)} - \frac{f(x_0)}{g(x_0)} &= \frac{f(x)g(x) - f(x_0)g(x_0)}{g(x)g(x_0)}\\
                &= \frac{1}{g(x)g(x_0)} \interv{(f(x)-f(x_0))\cdot g(x_0) + f(x_0)\cdot (g(x)-g(x_0))}\\
                \text{ Lemma 2 } \impl \exists \delta_0>0\colon \abs{g(x)} &\geq \frac{1}{2}\abs{g(x_0)}\tag{$x\in D, \abs{x-x_0} < \delta$}\\
                \intertext{f,g stetig in $x_0$. geg. $\varepsilon > 0$}
                \forall\varepsilon >0~\exists \delta_1, \delta_2 > 0\colon \abs{f(x)-f(x_0)} &< \frac{\varepsilon\cdot \abs{g(x)}}{4(1+\abs{g(x)})}\\
                \abs{g(x)-g(x_0)} &< \frac{\varepsilon\cdot \abs{g(x_0)}^2}{4(1+\abs{f(x)})}\\
                \impl \abs{\frac{f(x)}{g(x)}- \frac{f(x_0)}{g(x_0)}} &\leq \frac{1}{\abs{g(x)} \cdot \abs{g(x_0)}} \interv{??}\\
                &< \frac{2}{\abs{g(x)}^2}\cdot\interv{\frac{\varepsilon\cdot g(x_0)^2}{4(1+\abs{g(x)})} + \frac{\abs{f(x_0)}\cdot \abs{g(x_0)}}{4(1+\abs{f(x)})}} \leq \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon
            \end{align*}
        \end{proof}
    \end{satz}
\end{document}
