...
\begin{document}
    ...
    \section{$\realnumbers^d$ und Konvergenz von Folgen in $\realnumbers^d$}

    \subsection{Die komplexen Zahlen $\complexnumbers$ und der Raum $\complexnumbers^d$}
    \thispagestyle{pagenumberonly}

    \begin{align*}
        \realnumbers^d &\definedas\text{ Vektorraum der reellen d-Tupel}\\
        &=\set{\pair{x_1, x_2, \dots, x_d} \middle | x_j\in\realnumbers }\\
        x&= \pair{x_1, \dots, x_d}\\
        y&= \pair{y_1, \dots, y_d}\\
        \alpha, \beta &\in \realnumbers\\
        \alpha x + \beta y &\definedas \pair{\alpha x_1 + \beta y_1, \alpha x_2 + \beta y_2, \dots, \alpha x_d + \beta y_d}
        \intertext{Für $d=2$}
        \text{Euklidische Länge: }\abs{\abs{x}}^2 &= (x_1)^2 + (x_2)^2\\
        \abs{\abs{x}} &= \sqrt{(x_1)^2 + (x_2)^2}\\
        \intertext{Allgemein}
        \abs{\abs{x}} &\definedas \sqrt{\pair{\sum_{j=1}^{d} x_j ^2}}\tag{Def. der Euklidischen Länge eines Vektors $x\in\realnumbers^d$}
        \intertext{Wir schreiben auch}
        \abs{\abs{x}}_2 &= \abs{\abs{x}} = \sqrt{\pair{\sum_{j=1}^{d} x_j ^2}}
        \intertext{Alternativ}
        \abs{\abs{x}}_1 &\definedas \sum_{j=1}^{d} \abs{x_d} \tag{Manhattan-Norm}\\
        \abs{\abs{x}}_{\infty} &\definedas \max_{1\leq j \leq d} \abs{x_j}\tag{Maximumsnorm}\\
        \abs{}\cdot\abs{}_1, &\abs{}\cdot\abs{}_{\infty}, \abs{}\cdot\abs{}\text{ sind äquivalent}\\
        \abs{x_k} &\leq \sqrt{\sum_{j=1}^{d} \abs{x_j}^2} = \abs{\abs{x}}_2\tag{$1\leq k \leq d$}\\
        \impl \abs{\abs{x}}_{\infty} &= \max_{k=1,\dots, d} \abs{x_k} \leq \abs{\abs{x}}_2
        \intertext{Umgekehrt}
        \abs{\abs{x}}_2^2 &= \sum_{j=1}^{d} \abs{x_j}^2 \leq \sum_{j=1}^{d} \pair{\max_{k=1,\dots, d}\pair{\abs{x_k}}}^2 = d \cdot \abs{\abs{x}}_{\infty}^2\\
        \abs{\abs{x}}_{\infty} &\leq \abs{\abs{x}}_2 \leq \sqrt {d}\cdot \abs{\abs{x}}_{\infty}\\
        \impl \frac{1}{\sqrt{d}}\cdot \abs{\abs{x}}_2 &\leq \abs{\abs{x}}_2 \leq \abs{\abs{x}}_{\infty} \leq \abs{\abs{x}}_2\\
        \intertext{Ähnlich mit $\abs{\abs{x}}_1$ und $\abs{\abs{x}}_\infty$}
    \end{align*}

    \begin{definition}[Abstand zwischen Vektoren im $\realnumbers^d$] % Definition 1
        ...
        mit
        \begin{enumerate}[label=\alph*)]
            \item $\abs{\abs{x}} \geq 0~\forall x\in\realnumbers^d$ und\\ $\abs{\abs{x}} = 0\impl x=0$
            \item $\forall\lambda\in\realnumbers,~x\in\realnumbers^d\colon \abs{\abs{\lambda x}} = \abs{\lambda}\abs{\abs{x}}$
            \item $\forall x,y\in\realnumbers^d\colon\abs{\abs{x+y}} \leq \abs{\abs{x}} + \abs{\abs{y}}$\aligntoright{(Dreiecksungleichung)}{0.15}
        \end{enumerate}
    \end{definition}

    \begin{bemerkung}
        a), b) und c) sind für $\norm{.}_1, \norm{.}_{\infty}$ einfach zu zeigen. Außerdem ist a), b) einfach zu zeigen für $\norm{.}_2$, c) ist ??.\\
        Zu jeder Norm auf $\realnumbers^d$ (oder reellen Vektorräumen) Abstand von 2 Vektoren $x,y\in\realnumbers^d$ defineren wir als $\norm{x-y}$ (Abstand von $x$ und $y$).

        \begin{align*}
            d(x,y) &\definedas \norm{x-y}
            \intertext{Mit $z$ als weiterem Vektor}
            \norm{x-y} &= \norm{x-z+z-y}\\
            &\leq \norm{x-y} + \norm{z-y}\\
            \norm{x-y} &= \norm{-(y-x)} = \abs{-1\norm{y-x}} = \norm{y-x}
        \end{align*}
    ...
    \begin{folgerung}
        Bisher basierte unser Konvergenz-Begriff auf dem Abstand von $x_n$ zu $x$ kleiner als $\varepsilon$ für alle $n\geq N$.\\
        Wir verallgemeinern das für eine offene Kugel in $\realnumbers^d$ und $x\in\realnumbers^d$
        \begin{align*}
            B_\varepsilon(x) &\definedas \set{x\in\realnumbers^d\middle | \norm{x-y}_2 < \varepsilon}\tag{Mehrdimensionale $\varepsilon$-Umgebung}\\
            \norm{x-y}_2&=\sqrt{(x_1-y_1)^2 + (x_2 - y_2)^2} < \varepsilon\\
            \abs{x_1-y_1}^2 + \abs{x_2-y_2}^2 &< \varepsilon^2
            \intertext{In der Visualisierung erhalten wir einen offenen Ball um $x$ mit Radius $\varepsilon$}
        \end{align*}
    \end{folgerung}

    \begin{definition} % Definition 2
        Sei $(x_n)_n$ eine Folge in $\realnumbers^d$, $x_n\in\realnumbers^d~\forall n$. $(x_n)_n$ konvergiert gegen $x\in\realnumbers^d$, falls
        \begin{align*}
            \forall\varepsilon > 0~\exists N\in\naturalnumbers\colon& \norm{x_1-x}_2 <\varepsilon\quad\forall n\geq N
            \intertext{Das heißt}
            \forall\varepsilon > 0~\exists N\in\naturalnumbers\colon& x_1\in B_{\varepsilon}(x)\quad\forall n\geq N
        \end{align*}
    \end{definition}

    \begin{beobachtung}
        $\realnumbers^d\ni x = (x_n^1, x_n^2, \dots, x_n^d)$ (Koordinaten von $x_n$)\\
        $\impl$ Folgen $(x_n^1)$, $(x_n^2)$, \dots, $(x_n^d)$ in $\realnumbers$\nn
        Angenommen $x_n$ konvergiert gegen $x\in\realnumbers^d$. Dann konvergieren alle Folgen $(x_n^l)_n$ in $\realnumbers$ und $\lim_{\ntoinfty} x_n^l = x^l$
        ...
        \begin{proof}
            ...
            \end{align*}
            Also konvergiert $(x_n^l)$ gegen $x^l$ in $\realnumbers$.
        \end{proof}
    ...
    \begin{satz}
        ...
        \begin{proof}
            ...
            \anf{$\Leftarrow$} Angenommen $\forall l=1,\dots, d~\exists x^l \definedas \lim_{\ntoinfty} x^l_n$\\
            Das heißt $\forall l=1,\dots, d\colon \abs{x_n^l - x^l} \fromto 0$ für $n\fromto\infty$.\\
            Das heißt $\forall\varepsilon > 0~\exists N_l\in\naturalnumbers\colon \abs{x_n^l - x^l} < \frac{\varepsilon}{\sqrt{d}}\quad\forall n\geq N_{l}$
            \begin{align*}
                N&\definedas\max\pair{N_1, N_2, \dots, N_d}\\
                \norm{x_n-x}^2_2 &= \sum_{j=1}^{d} \abs{x_n^j - x^j}^2 < \sum_{j=1}^{d} \pair{\frac{\varepsilon}{\sqrt{d}}}^2\\
                &= d\cdot\frac{\varepsilon^2}{d} = \varepsilon^2\quad\forall n\geq N\\
    ...
    \begin{align*}
        ...
        &= x + y\cdot i
    \end{align*}
    $\complexnumbers = \realnumbers^2$ mit obiger Multiplikaton und Addition. Wir identifizieren $\realnumbers$ mit $\realnumbers\times\set{0}$ als Teilmenge von $\complexnumbers$.\\
