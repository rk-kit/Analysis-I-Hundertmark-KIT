...
\begin{document}
    ...
    \begin{definition}[Exponentialfunktion]
        Es sei $z\in\complexnumbers$. Dann gilt
        \begin{align*}
    ...
    \begin{satz}[Eigenschaften der Exponentialfunktion]
        \theoremescape
        \begin{enumerate}[label=(\alph*)]
            \item Für alle $z\in\complexnumbers$ konvergiert die obige Reihe absolut. (Wohldefiniertheit der $\exp$-Funktion)
            ...
            \item $e^x = \exp(x) > 0\quad\forall x\in\realnumbers$
        \end{enumerate}
    ...
    \begin{satz}[Definition von Sinus und Kosinus über Expontentialfunktionen]
        Für $\alpha\in\realnumbers$ ist $\abs{\exp(i\alpha)}=1$. Wir setzen
        \begin{align*}
            ...
            \intertext{Außerdem gilt $\forall \alpha\in\realnumbers$}
    ...
    \thispagestyle{pagenumberonly}
    Sei $D\subseteq \realnumbers$ (oder $D\subseteq \complexnumbers$, wenn wir eine komplexe Funktion betrachten) und $D\neq\emptyset$. Außerdem sei $\mathbb{K} = \realnumbers$ oder $\complexnumbers$ (Körper).

    \begin{definition}[$\varepsilon-\delta$ Definition von Stetigkeit]
        Sei $f: D \fromto \mathbb{K}$. Die Funktion $f$ heißt stetig in $x_0$, falls
        \begin{align*}
    ...
    \begin{bemerkung}
        ...
        \begin{enumerate}[label=\arabic*)]
            ...
            \item Eine Funktion $f: D\fromto\mathbb{K}^d$ ist stetig in $x_0\in D$, falls
            \begin{align*}
    ...
    \begin{beispiel}[Dirichlet-Funktion]
        ...
        \begin{align*}
            ...
            \end{cases}
        \end{align*}
        ist nirgends stetig auf $\realnumbers$.
    \end{beispiel}
    \begin{beispiel}
        Die Funktion
        \begin{align*}
    ...
    \begin{lemma} % Lemma 2
        ...
        Ist $f:D\fromto\mathbb{K}$ (oder $\mathbb{K}^d$) stetig in $x_0\in D$. Dann gilt
        \begin{enumerate}[label=\arabic*.]
    ...
    \begin{notation}
        ...
        und setzen
        \begin{align*}
            f + g: D_{f+g} &\fromto \mathbb{K},\quad x \mapsto  f(x) + g(x)\\
            f \cdot g: D_{f+g} &\fromto \mathbb{K},\quad x \mapsto  f(x) \cdot g(x)\\
            \frac{f}{g}: D_{\frac{f}{g}} &\fromto \mathbb{K},\quad x \mapsto \frac{f(x)}{g(x)}
        \end{align*}
    \end{notation}

    \begin{bemerkung}
        $f+g$ geht auch für $f: D_f \fromto \mathbb{K}^d$, $g: D_g \fromto \mathbb{K}^d$. $f\cdot g$ und $\frac{f}{g}$ gehen auch für $f: D_f \fromto \mathbb{K}^d$, $g: D_g \fromto \mathbb{K}$.
    \end{bemerkung}

    \newpage

    \begin{satz} % Satz 3
        Seien $f, g: D\fromto \mathbb{K}$ stetig in $x_0\in D$. Dann gilt
        \begin{enumerate}[label=\arabic*.]
            \item $f+g: D\fromto\mathbb{K},~x\mapsto f(x)+g(x)$ ist stetig in $x_0$.
            \item $f\cdot g: D\fromto\mathbb{K},~x\mapsto f(x)\cdot g(x)$ ist stetig in $x_0$.
            \item Ist $g(x_0)\neq 0$, dann ist $\frac{f}{g}: D_{\frac{f}{g}}\fromto\mathbb{K},~x\mapsto \frac{f(x)}{g(x)}$ stetig in $x_0$.
        \end{enumerate}
        ...
        \begin{proof}[Beweis (3.)]
            Sei $g(x_0) \neq 0$
            \begin{align*}
                \frac{f(x)}{g(x)} - \frac{f(x_0)}{g(x_0)} &= \frac{f(x)\cdot g(x_0) - f(x_0)\cdot g(x)}{g(x)\cdot g(x_0)}\\
                &= \frac{1}{g(x)\cdot g(x_0)}\cdot\interv{\pair{f(x)-f(x_0)}\cdot g(x_0) + f(x_0)\cdot \pair{g(x)-g(x_0)}}\tag{*}
                \intertext{Aus Lemma~\ref{lemma:abschaetzung-stetigkeit} folgt}
                \exists \delta_0>0\colon \abs{g(x)} &\geq \frac{1}{2}\abs{g(x_0)}\tag{$x\in D, \abs{x-x_0} < \delta$}\\
                \intertext{Nach der Stetigkeit von $f$ und $g$ gilt außerdem}
                \forall\varepsilon >0~\exists \delta_1, \delta_2 > 0\colon \abs{f(x)-f(x_0)} &< \frac{\varepsilon\cdot \abs{g(x_0)}}{4\cdot (1+\abs{g(x)})}\\
        ...
        \begin{proof}
            Es reicht aus, zu zeigen, dass $\frac{1}{g(x)}$ in $x_0$ stetig ist.
            \begin{align*}
                \abs{\frac{1}{g(x)} - \frac{1}{g(x_0)}} &= \frac{\abs{g(x)-g(x_0)}}{g(x)\cdot g(x_0)}
            \end{align*}
        \end{proof}
    \end{satz}
    ...
    \begin{satz}[Stetigkeit von Polynomen]
        \marginnote{[23. Jan]}
        Für eine stetige Funktion $f$ und $a$ eine Zahl, dann ist auch $a\cdot f$ stetig.\\
        Außerdem ist $x^2$ bzw. $z^2$ nach Multiplikation stetig. $\annot{\impl}{Ind.}$ $x^n$ bzw. $z^n$ stetig.\\
        Damit sind auch alle Polynome stetig auf ganz $\realnumbers$ bzw. $\complexnumbers$.\\
        Außerdem ist für $P,Q$ Polynome mit $Q(x)\neq 0$ auch die Funktion
        \begin{align*}
            R(x) &= \frac{P(x)}{Q(X)}
        \end{align*}
        stetig.
    ...
    \begin{uebung}
        Weisen Sie basierend auf der vorherigen Gleichung die Stetigkeit der Funktion $e^x$ nach.
    \end{uebung}

    \begin{korollar}
        Alle Polynome (reell oder komplex) sind stetig und alle polynomiellen Funktionen $P(x) = \frac{R(x)}{Q(x)}$ sind stetig auf $D_R = \mathbb{K}\exclude\set{x\in\mathbb{K}: Q(x) = 0}$.
    \end{korollar}

    \begin{satz} % Satz 5
        Sei $f: D\fromto\realnumbers$ (oder $\complexnumbers$, $\realnumbers^d$, $\complexnumbers^d$) mit $D\subseteq\realnumbers$ (oder $\complexnumbers$).\\
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                \forall \varepsilon > 0~\exists N\in\naturalnumbers\colon \abs{x_n-x_0} < \varepsilon\quad\forall n\geq N\\
                ...
                \impl \exists N\in\naturalnumbers\colon \abs{x_n-x_0} < \delta\quad\forall n\geq N\\
            ...
            \begin{align*}
                ...
                \intertext{Wählen $\delta = \frac{1}{n}$, $n\in\naturalnumbers$}
    ...
    \begin{satz} % Satz 6
        ...
        Sei $f: D_f \fromto\realnumbers$, $D_g\subseteq\realnumbers$, $g: D_g\fromto\realnumbers$. und $f(D_f)\subseteq D_g$.\\
        Sind $f$ stetig in $x_0$ und $g$ stetig in $y_0=f(x_0)\in D_g$. Dann ist $g\circ f: D_f\fromto\realnumbers$ stetig in $x_0$.
    ...
    % Erklärung für den Zwischenwertsatz

    \begin{beispiel}
        Die Funktion
        \begin{align*}
            f: \mathbb{Q} &\fromto \realnumbers\\
            x &\mapsto
            \begin{cases}
                1\quad x^2 &> 2\\
                -1\quad x^2 &< 2
            \end{cases}
        \end{align*}
        ist eine stetige Funktion auf $\mathbb{Q}$.
    \end{beispiel}

    \begin{satz}[Zwischenwertsatz] % Satz 1
        Sei $f: \interv{a,b}\fromto\realnumbers$ stetig, $f(a)\neq f(b)$. Dann gibt es zu jedem $c$ zwischen $f(a)$ und $f(b)$ (d.h. $f(a) < c < f(b)$ oder $f(b) < c < f(a)$) ein $\zeta$ zwischen $a$ und $b$ ($a<\zeta < b$) mit $f(\zeta) = c$.\\
        Dieser Satz funktioniert nicht auf $\complexnumbers$, da wir die Anordnung der reellen Zahlen verwenden und funktioniert nach dem vorherigen Beispiel nicht auf unvollständigen Mengen.
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                \end{split}
                \intertext{Setzen $\zeta\definedas \sup M\in\realnumbers$. Behauptung: $f(\zeta) = 0$. Beobachtung: $f$ ist stetig auf $\interv{a,b}$ und angenommen $f(\zeta)\neq 0$}
    ...
    \begin{definition} % Definition 1
        Eine Menge $A\subseteq\realnumbers$ (oder $\complexnumbers$) ist beschränkt, falls
        \begin{align*}
            \exists R>0\colon A\subseteq\pair{-R, R}
            \intertext{bzw}
            A\subseteq B_R(?) \definedas\set{z\in\complexnumbers: \abs{z} < R}
        \end{align*}
    ...
    \begin{definition} % Definition 2
        $K\subseteq\realnumbers$ (oder $\complexnumbers$) ist kompakt, falls jede Folge $(x_n)_n\subseteq K$ eine in $K$ konvergente Teilfolge besitzen. Das heißt
        \begin{enumerate}[label=(\alph*)]
    ...
    \begin{satz} % Satz 3
        $K\subseteq\realnumbers$ (oder $\complexnumbers$) ist genau dann kompakt, wenn $K$ abgeschlossen und beschränkt ist.
        ...
        \begin{proof}
            ...
            $K$ ist auch beschränkt. Wenn $K$ nicht beschränkt ist existiert für jedes $n\in\naturalnumbers$ ein $x_n\in K$ mit $\abs{x_n} > n$.\\
            Die Folge $(x_n)_n$ hat dann aber keine konvergente Teilfolge.
        \end{proof}
    \end{satz}

    \begin{satz} % Satz 4
        Sei $K\subseteq\realnumbers$ (oder $\complexnumbers$) und $f: K\fromto\realnumbers$ (oder $\complexnumbers$) stetig. Dann ist $f(K)$ kompakt.
        \begin{proof}
            Sei $(y_n)_n$ Folge in $f(k)$. $f(K) \definedas\set{f(x): x\in K}$\\
            $\impl \forall n\in\naturalnumbers~\exists x_n\in K\colon f(x_n) = y_n$. Nach Definition existiert eine Teilfolge $(x_{n_j})_j$ von $(x_n)_n$, welche gegen ein $x_0\in K$ konvergiert.
            \begin{align*}
