...
\begin{document}
    ...
    \section{[*] Konvexität}
    \thispagestyle{pagenumberonly}
    \imaginarysubsection{[*] Konvexität}
    ...
    \begin{skizze}
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                \draw[->] (-2, 0) -- (2, 0);
                \draw[->] (0, -2) -- (0, 2);
                \draw[scale=1, domain=-2:2, smooth, variable=\x] plot ({\x}, {\x});
            \end{tikzpicture}
            \caption{}
        \end{figure}

        Wähle ein $\lambda\in\pair{0,1}$ und formuliere die Interpolation $r=\lambda x + \pair{1-\lambda}y$
    \end{skizze}

    \begin{definition}[Konvexität/Konkavität]
        Sei $f: D\fromto \R$ wobei $D\subseteq \R$ ein Intervall. Die Funktion $f$ heißt konvex (bzw. konkav), falls für alle $x,y\in D$ und alle $\lambda \in\pair{0,1}$ gilt
        \begin{align*}
            f\of{\lambda x + \pair{1-\lambda}y} \underbrace{\leq}_{\text{bzw. } \geq} \lambda f\of{x} + \pair{1-\lambda} f\of{y}
        \end{align*}
    \end{definition}

    \begin{beispiel}
        Wir betrachten $f: \R\fromto\R$ mit $x\mapsto x^2$. Sei $x,y\in\R$ und \OBDA sei $y\geq x$, sei $\lambda\in\pair{0,1}$. Dann ist zu zeigen
        \begin{align*}
            f\of{\lambda x + \pair{1-\lambda}y} &\leq \lambda f\of{x} + \pair{1-\lambda} f\of{y}\\
            \equivalent \pair{x+\pair{1-\lambda}y}^2 &\leq \lambda x^{2} + \pair{1-\lambda}y^2\\
            \equivalent \lambda^2 x^2 + 2\lambda\pair{1-\lambda}xy + \pair{1-\lambda}^{2y^2} & \leq \lambda x^2 + \pair{1-\lambda}y^2\\
            \equivalent \lambda\pair{\lambda -1}x^2 + \pair{1-\lambda}\pair{1-\lambda-1} y^2 + \lambda\pair{1-\lambda} 2xy &\leq 0\\
            \equivalent \underbrace{\lambda\pair{\lambda -1}}_{\leq 0}\interv{x^2+y^2+2xy} &\leq 0\\
    ...
    \begin{korollar}[Ungleichung von Hölder] % Korollar 2
