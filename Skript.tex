...
\begin{document}
    ...
    \begin{beispiel}[Eulersche Zahl über Reihendarstellung]
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                \intertext{Halte $m\in\naturalnumbers$ fest}
                \impl e &= \lim_{\ntoinfty} \pair{1+\frac{1}{n}}^n \geq \lim_{\ntoinfty} \sum_{k=0}^{m} \frac{1}{k!}\cdot \prod_{j=0}^{k-1} \pair{1-\frac{j}{n}}\\
                &= \sum_{k=0}^{m} \frac{1}{k!} \cdot \prod_{j=0}^{k-1} \lim_{\ntoinfty} \pair{1-\frac{j}{n}}
                \intertext{Der Limes ist 1:}
    ...
    \begin{bemerkung}
        \begin{align*}
            \sum_{n=0}^{\infty} \frac{x^n}{n!} &\definedasbackwards a_n\\
            \frac{\abs{a_{n+1}}}{\abs{a_n}} &= \abs{\frac{x^{n+1}\cdot n!}{(n+1)!\cdot x^n}} = \frac{\abs{x}}{n+1}\fromto 0
        \end{align*}
    \end{bemerkung}

    \begin{satz}[Lambert 1707] % Satz 7
        $e$ ist irrational.
        \begin{proof}
            Wir haben
            \begin{align*}
                \frac{1}{(n+1)!} \leq e-s_n \leq \frac{e-1}{(n+1)!} < \frac{2}{(n+1)!}
            \intertext{Angenommen $e=\frac{p}{q}$~$p$,$q\in\naturalnumbers$. Wir nehmen $n=q$}
            \impl \frac{1}{(q+1)!} &\leq \frac{p}{q} - s_q < \frac{2}{(q+1)!}\\
            0 &< \frac{1}{q+1} \leq \frac{p}{q}\cdot q! - q!\cdot s_q < \frac{2}{q+1}\\
    ...
    \begin{beispiel}
        \begin{align*}
            ...
            &= \frac{1}{(n+1)!}\cdot\interv{1+\frac{1}{n+2}+\frac{1}{(n+2)\cdot(n+3)} + \dots + \frac{1}{(n+2)\cdot(n+3)\cdot\dots\cdot(n+p)}}\\
            &> \frac{1}{(n+1)!}\\
            &1+\frac{1}{n+2}+\frac{1}{(n+2)\cdot(n+3)} + \dots + \frac{1}{(n+2)\cdot(n+3)\cdot\dots\cdot(n+p)}\\
            &< 1 + \frac{1}{2} + \frac{1}{2\cdot 3}+\dots + \frac{1}{2\cdot 3 \cdot\dots\cdot p}\\
            &= \sum_{k=0}^{\infty} \frac{1}{k!} - 1 < ??? = e-1\\
            \impl \frac{1}{(n+1)!} &< r_{n,p} < \frac{e-1}{(n+1)!}\\
            \frac{1}{(n+1)!} &< s_{n+p} - s_{n} < \frac{e-1}{(n+1)!}\\
            s_{n+p} - s_{n} &\fromto e - s_n
        \end{align*}
    \end{beispiel}

    \begin{satz}[Wurzelkriterium]
        \label{satz:wurzelkriterium} % Satz 8
        ...
        \begin{proof}[Beweis (iii)]
            \begin{align*}
                ...
                \sum_{n=1}^{\infty} \frac{1}{n^p}\text{ divergiert für $p=1$, konvergiert für $p>1$}
                \impl \text{ keine Aussage möglich}
            \end{align*}
        \end{proof}
        \begin{proof}[Beweis (i)]
            ...
            \begin{align*}
                ...
                \impl \sqrt[n]{\abs{a_n}} < \hat{q} + \varepsilon = \frac{1+\hat{q}}{2}\text{für fast alle $n$}\\
                \impl \exists n_{0}\in\naturalnumbers\colon \sqrt[n]{\abs{a_n}} \leq q\quad\forall n\geq n_0\\
                \equivalent \abs{a_n} \leq q^n\quad\forall n\geq n_0\\
                \intertext{Das heißt $\sum_{n=0}^{\infty} q^n$ ist konvergente Majorante für $\sum a_n$}
                \impl \sum_{n=0}^{\infty} a_n\text{ ist absolut konvergent}
            \end{align*}
        \end{proof}
        \begin{proof}[Beweis (ii)]
            Sei
            \begin{align*}
                ...
                \impl (a_n)_n\text{ keine Nullfolge somit konvergiert $\sum a_n$ nicht}
            \end{align*}
        \end{proof}
    \end{satz}
