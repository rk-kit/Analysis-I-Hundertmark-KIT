...
\begin{document}
    ...
    \begin{satz} % Satz 2
        \label{satz:18-2}
        Sei $a\in\pair{c,d}\sbset\R$. Die Funktion $f: \pair{a,b}\fromto\R$ ist genau dann im Punkt $a$ differenzierbar, wenn $\ex C\in\R$ mit $f(x) = f(a)+C\cdot\pair{x-a} + \varphi\of{x}$ wobei
        \begin{align*}
    ...
    \begin{korollar}
        ...
        \label{korollar:18-2}
        \begin{enumerate}[label=(\roman*)]
            \item $f$ ist differenzierbar $\impl f$ ist stetig im Punkt $a$.
        ...
        \begin{proof}[Beweis (i)]
            \begin{align*}
                x\fromto a, f(x)-f(a) = C\cdot\pair{x-a} + \frac{\varphi(x)}{x-a}\cdot\pair{x-a}\fromto 0
            \end{align*}
        \end{proof}
        ...
        \begin{proof}[Beweis (ii)]
            \begin{align*}
                ...
                \abs{f(x)-f(a)} &\geq \abs{f'(a)}\abs{x-a} = \underbrace{\abs{\frac{\varphi(x)}{x-a}}}_{\fromto 0}\cdot\abs{x-a}\geq \frac{1}{2}\abs{f'(a)}\abs{x-a}
            \end{align*}
        \end{proof}
        \begin{proof}[Beweis (iii)]
            1.
            \begin{align*}
                \abs{f(x)-f(a)} &\leq \abs{f'(a)}\abs{x-a} + \underbrace{\frac{\varphi(x)}{x-a}}_{\fromto 0} \cdot \abs{x-a} \leq 2\abs{f'(a)}\cdot\abs{x-a}
                \intertext{2.}
                \abs{f(x)-f(a)} &\leq \abs{\underbrace{f'(a)}_{\fromto 0} \cdot \pair{x-a} + \underbrace{\frac{\varphi(x)}{x-a}}_{\fromto 0}\cdot\pair{x-a}} \leq \varepsilon\abs{x-a}
            \end{align*}
        \end{proof}
    \end{korollar}
    ...
    \begin{satz}[Kettenregel] % Satz 4
        ...
        \begin{proof}
            ...
            \begin{align*}
                ...
                \impl \lim_{h\fromto 0} \frac{F(x+h)-F(x)}{h} &= \lim_{h\fromto 0} \frac{\pair{F(x+h) - F(x)}\cdot\pair{f(x+h)-f(x)}}{\pair{f(x+h)-f(x)}h}\\
                ...
                \abs{\Delta y} &\leq \frac{1}{2}\abs{f'(x)}\cdot\abs{x+h-x} \fromto 0\\
                \lim_{h\fromto 0} \frac{g(y+\Delta y) - g(y)}{\Delta y} &= \lim_{\Delta y \fromto 0} \frac{g(y+\Delta y) - g(y)}{\Delta y} = g'(y) = g'(f(x))\\
                \impl \frac{F(x+h)-F(x)}{h} \fromto g'(f(x)) \cdot f'(x)
                \intertext{Fall 2: $f'(x) = 0$}
                \lim_{h\fromto 0} \frac{F(x+h)-F(x)}{h}\\
                ...
                \impl \lim_{h\fromto 0} \frac{F(x+h)-F(x)}{h} &= 0\\
                f'(x) = 0 \impl \lim_{h\fromto 0} \frac{g(f(x+h))-g(f(x))}{h} = 0\\
    ...
    \begin{beispiel}
        ...
        \begin{align*}
            \pair{e^{ax}}' &= \lim_{h\fromto 0} \frac{e^{a(x+h)} - e^{ax}}{h} = \lim_{h\fromto 0}e^{ah} \cdot \frac{e^{ax}-1}{h} = e^{ah}\cdot \lim_{h\fromto 0} \frac{e^{ah}-1}{h}\\
            e^{ah} &= \sum_{n=0}^{\infty} \frac{(ah)^n}{n!} = 1 + ah + (ah)^2 \sum_{n=2}^{\infty} \frac{(aH)^(n-2)}{n!}
            \intertext{$\abs{h} < \frac{1}{\abs{a}}$}
            ... &= e^{ax} \lim_{h\fromto 0} \frac{ah}{h} = a\cdot e^{ax}\tag{$a\in\C$}
        \end{align*}
    \end{beispiel}
    ...
    \begin{beispiel}[Ableitung von $\tan$]
        \begin{align*}
            \pair{\tan x}' &= \pair{\frac{\sin x}{\cos x}}' = \frac{\cos x \cdot \cos x -\pair{-\sin x \cdot \sin x}}{\cos^2 x} = \frac{\cos^2 x + \sin^2 x}{\cos^2 x} \frac{1}{\cos^2 x} = \frac{1}{\cos^2 x}
        \end{align*}
    \end{beispiel}
    ...
    \begin{satz}[Ableitung der Umkehrfunktion] % Satz 6
        ...
        \begin{proof}
            \begin{align*}
                \lim_{h\fromto 0} \frac{\overbrace{f^{-1}\of{y+h}}^{\definedasbackwards \xi} - f^{-1}\of{y}}{h} &= \lim_{h\fromto 0}\frac{\xi - x}{f\of{\xi} - f\of{x}}\\
                &= \lim_{h\fromto 0} \frac{1}{\frac{f\of{\xi} - f\of{x}}{\xi-x}}\\
                &= \frac{1}{\lim_{h\fromto 0} \frac{f\of{\xi} - f\of{x}}{\xi-x}}
                \intertext{Wegen der Monotonie und Stetigkeit von $f$ können wir umformen zu}
                &= \frac{1}{\lim_{\xi\fromto x} \frac{f\of{\xi} - f\of{x}}{\xi-x}}\\
                &= \frac{1}{f'\of{x}}\qedhere
            \end{align*}
        \end{proof}
    \end{satz}
    ...
    \begin{beispiel}[Ableitung von $\arccos$]
        ...
        \begin{align*}
            \pair{\arccos y}' &= \frac{1}{\pair{\cos x}'_{\lvert \cos x = y}} = \frac{1}{-\sin\of{\cos\of{x}}}\\
    ...
    \begin{satz}[Mittelwertsatz]
        ...
        \begin{proof}
            \begin{align*}
                F(x) &= f(x) - \frac{f\of{b}-f\of{a}}{b-a}\cdot\pair{x-a}\\
                F(a) &= f(a) - \frac{f(b)-f(a)}{b-a}\cdot 0\\
                ...
                \impl F'\of{\xi} &= f'\of{\xi} - \frac{f\of{b} - f\of{a}}{b-a} = 0\\
    ...
    \begin{bemerkung}
        Der Mittelwertsatz sagt aus, dass es einen Punkt auf der Funktion gibt, dessen Tangente parallel zu einer affin-linearen Funktion durch $f(a)$ und $f(b)$ läuft. % TODO: vis
    \end{bemerkung}
    ...
    \begin{korollar} % Korollar 10
        ...
        \begin{proof}
            \begin{align*}
                f(x_1) - f(x_2) \annot[{&}]{=}{\ref{satz:mittelwertsatz}} f'\of{\xi}\cdot\pair{x_1 - x_2} = 0\cdot\pair{x_1 - x_2}\\
    ...
    \begin{satz} % Satz 11
        ...
        \begin{proof}
            \begin{align*}
                h(t) &= \interv{f(b)-f(a)}\cdot g(t) - \interv{g(b)-g(a)}\cdot f(t)\\
                \impl h(a) &= h(b)\\
                \impl \ex \xi \in\pair{a,b} &\text{ mit } h'\of{\xi} = 0\\
    ...
    \begin{definition}[Ableitungen höherer Ordnung] % Definition 11
    ...
    \begin{satz}[Der Taylorsche Satz] % Satz 12
        Es sei $f$ eine reelle Funktion auf $\interv{a,b}$ und $f^{(n-1)}$ sei stetig auf $\interv{a,b}$ und $f^{(n)}$ existiere auf $\pair{a,b}$. Sei
        \begin{align*}
            P_n(t) &= \sum_{k=0}^{n-1} \frac{f^{(k)}\of{\alpha}}{k!}\cdot\pair{t-\alpha}^k
            \intertext{Dann $\ex\xi\in\pair{a,b}$ so dass}
            f\of{\beta} &= P_n\of{\beta} + \frac{f^{(n)}\of{\xi}}{n!}\cdot\pair{\beta-\alpha}^n
        \end{align*}
    \end{satz}

    \begin{bemerkung}
        Der Taylorsche Satz ist eine Verallgemeinerung des Mittelwertsatzes.
    \end{bemerkung}
